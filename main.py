...
if __name__ == '__main__':
    ...
    parser.add_argument('--val_raw_images_dir', type=str, required=True)
    parser.add_argument('--val_comp_images_dir', type=str, required=True)
    ...
    dataset = WaymoDataset(opt.raw_images_dir, opt.comp_images_dir, opt.patch_size)
    val_dataset = WaymoDataset(opt.val_raw_images_dir, opt.val_comp_images_dir, opt.patch_size)
    ...
    for epoch in range(opt.num_epochs):
        ...
        if val_losses.avg < best_val_loss:
            ...
            torch.save(model.state_dict(), 
                      os.path.join(opt.outputs_dir, f'{opt.arch}_best_clean_jpg50.pth'))
